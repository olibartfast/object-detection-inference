...
std::vector<cv::Mat> convertPreprocessedToMats(
    ...
    const std::vector<std::vector<int64_t>>& /*input_sizes*/) { // input_sizes ignored
    
    std::vector<cv::Mat> input_mats;
    input_mats.reserve(preprocessed.size());

    for (const auto& buffer : preprocessed) {
        // Create a 1D Matrix: 1 Row, N Columns, Type CV_8U (unsigned char)
        // This wraps the existing memory pointer. NO COPY takes place.
        cv::Mat wrapper(1, static_cast<int>(buffer.size()), CV_8U, 
                        const_cast<uint8_t*>(buffer.data()));
        
        input_mats.push_back(wrapper);
    }
    return input_mats;
}
