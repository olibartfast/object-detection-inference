...
YoloV8::YoloV8(const std::string& model_path, bool use_gpu,
    ...
    TRTInfer{model_path, use_gpu, confidenceThreshold,
            network_width,
            network_height}
{
...
std::vector<Detection> YoloV8::postprocess(const cv::Size& frame_size)
{

    const float*  output0 = h_outputs_.front().data();
    std::vector<int64_t> shape0 = output_shapes_.front();
